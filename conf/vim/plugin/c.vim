...
"       Revision:  $Id: c,v 1.131 2011/01/25 21:12:31 mehner Exp $
"
"------------------------------------------------------------------------------
"
if v:version < 700
  echohl WarningMsg | echo 'The plugin c-support needs Vim version >= 7 .'| echohl None
  finish
endif
"
...
if	s:MSWIN
  " ==========  MS Windows  ======================================================
	...
	if match( s:sourced_script_file, escape( s:vimfiles, ' \' ) ) == 0
		...
		let s:C_GlobalTemplateDir		= g:plugin_dir.'c-support/templates'
		let s:C_GlobalTemplateFile  = s:C_GlobalTemplateDir.'/Templates'
	else
		" user installation assumed
		"let g:plugin_dir  					= $HOME.'/vimfiles/'
	endif
	"
  ...
  " ==========  Linux/Unix  ======================================================
	...
	if match( expand("<sfile>"), expand("$HOME") ) == 0
		" user installation assumed
    if ! g:plugin_dir
  		let g:plugin_dir  	= g:plugin_ldir.'/'
    endif
	else
		" system wide installation
		let s:installation					= 'system'
	...
	"
	let s:C_LocalTemplateFile     = g:plugin_ldir.'/c-support/templates/Templates'
	let s:C_LocalTemplateDir      = fnamemodify( s:C_LocalTemplateFile, ":p:h" ).'/'
	let s:C_CodeSnippets  				= g:plugin_ldir.'/c-support/codesnippets/'
...
"  Key word completion is enabled by the filetype plugin 'c'
"  g:C_Dictionary_File  must be global
"
if !exists("g:C_Dictionary_File")
  let g:C_Dictionary_File = g:plugin_dir.'c-support/wordlists/c-c++-keywords.list,'.
        \                   g:plugin_dir.'c-support/wordlists/k+r.list,'.
        \                   g:plugin_dir.'c-support/wordlists/stl_index.list'
endif
"
...
let s:C_TypeOfH               = 'cpp'
let s:C_Wrapper               = g:plugin_dir.'c-support/scripts/wrapper.sh'
...
"-------------------------------------------------------------------------------
" callback functions used in the filetype plugin ftplugin/c
" callback functions
"-------------------------------------------------------------------------------
