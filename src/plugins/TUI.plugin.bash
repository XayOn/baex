...
mkmenufromcsvdir(){
    document "help" "Create a menu from a directory of csv files" "directory" && return
    read_csv $1 main.csv
    build_menu
}

read_csv() {
    unset menuopts menufuncs
    declare -gA menuopts menufuncs
    while IFS=, read opt func; do
        [[ -e $1/$opt ]] && read_csv $1 $opt || {
            menuopts[${#menuopts[@]}]=$opt;
            menufuncs[${#menufuncs[@]}]=$func;
        }
    done < $1/$2
}
